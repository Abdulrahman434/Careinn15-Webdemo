...
import { RemindersCalendarTool } from "./components/tools/RemindersCalendarTool";
import { WaterTrackerTool } from "./components/tools/WaterTrackerTool";
...
function BedsideScreen() {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      if (status.current !== Prayer.None && status.current !== lastPrayerRef.current) {
        ...
        // Skip Sunrise as it's not one of the 5 daily prayers requiring Azan
        if (lastPrayerRef.current !== Prayer.None && status.current !== Prayer.Sunrise) {
  ...
  const handlePrayerTimeReached = useCallback((pKey: PrayerType) => {
  ...
  return (
    ...
    >
      ...
      >
        ...
        {activeTool === "reminders-calendar" && <RemindersCalendarTool onClose={() => setActiveTool(null)} onBackToTools={() => { setActiveTool(null); setOpenCategory("Tools"); }} />}
        ...
        {activeTool === "todolist" && <ToDoListTool onClose={() => setActiveTool(null)} onBackToTools={() => { setActiveTool(null); setOpenCategory("Tools"); }} />}

        {activeTool === "watertracker" && <WaterTrackerTool onClose={() => setActiveTool(null)} onBackToTools={() => { setActiveTool(null); setOpenCategory("Tools"); }} />}
        {activeTool === "moodtracker" && <MoodTrackerTool onClose={() => setActiveTool(null)} onBackToTools={() => { setActiveTool(null); setOpenCategory("Tools"); }} />}
        {activeTool === "dailyquotes" && <DailyQuotesTool onClose={() => setActiveTool(null)} onBackToTools={() => { setActiveTool(null); setOpenCategory("Tools"); }} />}
      </div>
