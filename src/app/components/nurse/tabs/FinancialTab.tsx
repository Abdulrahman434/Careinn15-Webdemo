    ...
    <div className="space-y-5">
        ...
        <div className="nurse-card flex items-center justify-between" style={{ marginBottom: 0 }}>
          ...
          <label className="relative inline-flex items-center cursor-pointer">
            ...
            <div className="ni-switch"
              style={{ backgroundColor: store.sectionVisibility.financial ? t.primary : undefined }} />
          </label>
        </div>
      )}
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          ...
          { label: "Insurance Covered", value: totalCovered, color: t.successOn },
          { label: "Patient Responsibility", value: patientOwes, color: t.errorOn },
        ].map((item) => (
      ...
      <div className="nurse-card">
        <h3 style={{ color: t.textHeading }}><DollarSign size={18} style={{ color: t.primaryOn }} /> Breakdown</h3>
        <div className="overflow-hidden rounded-xl" style={{ border: `1px solid ${t.borderDefault}` }}>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "13px" }}>
            <thead>
              <tr style={{ backgroundColor: t.surfaceInset }}>
            ...
            <tbody>
              {store.financial.map((f) => (
                <tr key={f.id} style={{ borderBottom: `1px solid ${t.borderDefault}` }}>
                  ...
                  <td style={{ padding: "10px 14px", fontWeight: 700, color: t.successOn }}>SAR {f.covered.toLocaleString()}</td>
                  <td style={{ padding: "10px 14px", fontWeight: 700, color: t.errorOn }}>SAR {(f.amount - f.covered).toLocaleString()}</td>
                </tr>
              ))}
            </tbody>
          </table>
