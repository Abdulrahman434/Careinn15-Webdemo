        ...
        <div className="nurse-card flex items-center justify-between" style={{ marginBottom: 0 }}>
          ...
          <label className="relative inline-flex items-center cursor-pointer">
            ...
            <div className="ni-switch"
              style={{ backgroundColor: store.sectionVisibility.baby ? t.primary : undefined }} />
          </label>
        </div>
      )}

      <div className="nurse-card">
      <h3 style={{ color: t.textHeading }}><Baby size={18} style={{ color: t.primaryOn }} /> Baby Camera</h3>
      <div className="space-y-4">
        {store.babyCameras.map((cam) => (
          <div key={cam.id} className="rounded-2xl overflow-hidden" style={{ border: `1px solid ${t.borderDefault}`, opacity: cam.visible ? 1 : 0.4 }}>
            ...
            {/* Info */}
            <div className="flex items-center justify-between px-4 py-3" style={{ backgroundColor: t.surface }}>
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 rounded-full flex items-center justify-center" style={{ backgroundColor: t.primarySubtle }}>
                  <Baby size={16} style={{ color: t.primaryOn }} />
                </div>
                <div>
              ...
              <div className="flex items-center gap-3">
                {cam.connected && (
                  <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-full" style={{ backgroundColor: t.successSubtle }}>
                    ...
                    <span style={{ fontSize: "11px", fontWeight: 700, color: t.successOn }}>Connected</span>
                  </div>
                )}
                {isNurse && (
                  <button onClick={() => nurseActions.setBabyCameraVisible(cam.id, !cam.visible)}
                    ...
                    {cam.visible ? <Eye size={14} style={{ color: t.primaryOn }} /> : <EyeOff size={14} style={{ color: t.errorOn }} />}
                  </button>
                )}
              </div>
            </div>
